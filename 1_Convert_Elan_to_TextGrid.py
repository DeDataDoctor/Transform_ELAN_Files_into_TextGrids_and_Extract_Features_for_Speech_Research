...
for nr in range(0,len(DirectoryList)):
    for root, dirs, files in os.walk("PATH\\%s" %(DirectoryList[nr])): #Make sure the root here is correct and adjust it when needed.
        for file in files:
...
##Calculate duration in miliseconds for each audio file in the list. This step can take some time. We use miliseconds since pympi assumes this as normal input.
...
for nr in range(0,len(TextGridFileList)): # nr stands for a number that is part of the itteraction through a string of filename strings in FileList  
    shutil.copyfile(AudioFileList[nr], "PATH\\%s%s" %(GeneralFileNameList[nr], ".wav"))
    shutil.copyfile(TextGridFileList[nr], "PATH\\%s%s" %(GeneralFileNameList[nr], ".TextGrid"))
